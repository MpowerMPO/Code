...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in mpower.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("mpower.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Mpower features */
    { "mpower",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "mpower",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "mpower",               "gobject",                &gobject,                true,  {} },
    { "mpower",               "voteraw",                &voteraw,                true,  {} },
