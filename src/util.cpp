...
const char * const BITCOIN_CONF_FILENAME = "mpower.conf";
const char * const BITCOIN_PID_FILENAME = "mpowerd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "mpower" is a composite category enabling all Mpower-related debug output
                if(ptrCategory->count(std::string("mpower"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Mpower
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Mpower
    // Mac: ~/Library/Application Support/Mpower
    // Unix: ~/.mpower
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mpower";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mpower";
#else
    // Unix
    return pathRet / ".mpower";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty mpower.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override mpower.conf
