...
*Setup instructions for a Gitian build of Mpower Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mpower
...
- [Building Mpower Core](#building-mpower-core)
...
Clone the git repositories for Mpower Core and Gitian.
...
in the Mpower Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Mpower Core
----------------

To build Mpower Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Mpower Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mpower/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mpowerpay/mpower
...
For example:
```bash
URL=https://github.com/crowning-/mpower.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mpower=${COMMIT} --url mpower=${URL} ../mpower/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mpower=${COMMIT} --url mpower=${URL} ../mpower/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mpower=${COMMIT} --url mpower=${URL} ../mpower/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mpower git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mpower/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mpower
SIGPATH=/some/root/path/mpower-detached-sigs

./bin/gbuild --url mpower=${BTCPATH},signature=${SIGPATH} ../mpower/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mpower-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mpower-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mpower-osx-build.assert
```
...
[mpower/gitian.sigs](https://github.com/mpowerpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
